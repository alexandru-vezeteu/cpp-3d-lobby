...
PosixSocket::PosixSocket(PosixSocket &&other) noexcept : fd{other.fd}, s_addr{other.s_addr}, conn_addr(std::move(other.conn_addr)) {
    other.fd = -1;
    memset(&other.s_addr, 0, sizeof(other.s_addr));
}

PosixSocket::PosixSocket(const int domain, const int type, const int protocol) {
...
void PosixSocket::Bind(const std::string &addr, uint16_t port) {
    if (fd < 0) {
...
void PosixSocket::Listen(const int n) const {
    if (fd < 0) {
...
PosixSocket PosixSocket::Accept(const int options) const {
    if (fd < 0) {
...
void PosixSocket::Connect(const std::string &addr, const uint16_t port) {
    if (fd < 0) {
...
size_t PosixSocket::ReceiveFrom(void *buff, const size_t size, const int options, sockaddr *src_addr,
...
size_t PosixSocket::SendTo(const void *buff, const size_t size, const int options, const sockaddr *dest_addr,
                      const socklen_t addrlen) const {
    if (fd < 0) {
...
void PosixSocket::Shutdown(const int opt) {
    if (fd > 0) {
...
size_t PosixSocket::SendAll(const void *buffer, size_t size) const {
    size_t totalSent = 0;
...
size_t PosixSocket::RecvAll(void *buffer, size_t size) const {
...
bool PosixSocket::IsReadable(int ms) const {
    pollfd pfd;
    pfd.fd = fd;
    pfd.events = POLLIN;
...
bool PosixSocket::IsWriteable(int ms) const {
    pollfd pfd;
    pfd.fd = fd;
    pfd.events = POLLOUT;
